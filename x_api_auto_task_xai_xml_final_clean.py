...
def parse_llm_xml(xml_text: str) -> dict:
    ...
    # ── COVER (supports both attribute style and child-tag style) ──────────
    ...
    if cover_match:
        ...
        # child-tag style: <COVER><title>...</title>...</COVER>
    ...
    # ── THEMES ─────────────────────────────────────────────────────────────
    # Capture entire attribute string first, then parse individually —
    # this handles any attribute order, single/double/curly quotes, missing attrs.
    for tm in re.finditer(r'<THEME([^>]*)>(.*?)</THEME>', xml_text, re.I | re.S):
        ...
        # tweets — support both old style (no likes/replies) and new style
        tweets = []
    ...
    # ── INVESTMENT_RADAR / INVESTMENTRADAR ─────────────────────────────────
    def _extract_items(tag, target):
        bm = re.search(rf'<{tag}>(.*?)</{tag}>', xml_text, re.I | re.S)
        if bm:
            for im in re.finditer(r'<ITEM\b([^>]*)>(.*?)</ITEM>', bm.group(1), re.I | re.S):
                cat_m = re.search(r'category\s*=\s*[\'"\u201c\u201d](.*?)[\'"\u201c\u201d]', im.group(1), re.I)
                target.append({
                    "category": norm_text(cat_m.group(1)) if cat_m else "",
                    "content":  norm_text(im.group(2)),
                })

    _extract_items("INVESTMENT_RADAR",  data["investment_radar"])
    _extract_items("INVESTMENTRADAR",   data["investment_radar"])   # fallback tag
    _extract_items("RISK_CHINA_VIEW",   data["risk_china_view"])
    _extract_items("RISKCHINAVIEW",     data["risk_china_view"])    # fallback tag
